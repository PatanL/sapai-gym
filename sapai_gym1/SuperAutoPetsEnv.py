...
#     MAX_SHOP_PETS = 6
#     # Max number of foods that can be in a shop, probably should remove or change this
#     MAX_SHOP_FOODS = 3
...
#         self.observation_space = spaces.Box(low=0, high=255, shape=(len_obs_space,), dtype=np.uint8)
...
#         # Convert the numpy random state 
#         seed_state = self.rs_random.get_state()
    
#         self.player = Player(seed_state=seed_state)
...
#         self.bad_action_reward_sum = 0
#         encoded_state = self._encode_state()
#         print("Encoded state shape:", encoded_state.shape)
#         print("Encoded state dtype:", encoded_state.dtype)
#         print("Encoded state min:", encoded_state.min())
#         print("Encoded state max:", encoded_state.max())
#         return encoded_state, {}  # Return info dict as per Gymnasium API
...
#         if len([slot for slot in self.player.team if not slot.empty]) >= 5:
#             for team_idx, slot in enumerate(self.player.team):
#                 if slot.empty:
#                     continue
#                 action_num = self.ACTION_BASE_NUM["sell"] + team_idx
...
#     #     }
#     def _avail_reorder(self):
#         if self.just_reordered or self.player.gold > 0:
#             return {}
...
#         # TODO : FREEZE SHOP ITEMS, very important
...
#         masks = np.zeros(self.MAX_ACTIONS, dtype=bool)
#         for a in self._avail_actions().keys():
#             masks[a] = True
#         return masks
...
#         food_slots = []
#         for slot in self.player.shop._slots: #change back to shop_slots
...
#         # Other player stats
#         # Assumptions: Treat max gold as 20. Treat max turn as 25. Treat max cans as 10. Change assumptions from arena to versus (change lives), gold limit
...
#         state = np.concatenate(all_lists)
